...
const sidebarHTML = `
  ...
  <nav class="sidebar-nav">
    ...
    <a href="modulo_pedidos.html?bandeja=1" class="nav-item" id="nav-bandeja" title="Bandeja de pedidos" data-open-bandeja="1">
      <i class="fas fa-inbox"></i>
      <span>Bandeja de pedidos</span>
      <span id="badgeBandejaEnviarSb" class="bandeja-badge" title="Por enviar" style="display:none;">0</span>
      <span id="badgeBandejaAprobarSb" class="bandeja-badge apr" title="Por aprobar" style="display:none;">0</span>
    </a>
...
function ensureSidebarCollapseStyles() {
    ...
    style.textContent = `
      ...
      .bandeja-badge {
        ...
        border-radius: 999px; background: #8b3a4a; color: #fff;
        font-size: 0.65rem; font-weight: 700; align-items: center; justify-content: center;
        margin-left: 0.25rem;
      }
      .bandeja-badge.apr { background: #d97706; }
      .sidebar.collapsed .bandeja-badge { display: none !important; }
    `;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    else if (currentPath.includes('modulo_pedidos')) {
        document.getElementById('nav-pedidos')?.classList.add('active');
        if (window.location.search.includes('bandeja=1') || window.location.hash === '#bandeja') {
            document.getElementById('nav-bandeja')?.classList.add('active');
        }
    }
