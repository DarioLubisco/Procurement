...
RawCalculations AS (
  ...
  SELECT
    ...
    CAST(eq.origen AS VARCHAR(50)) AS origen,
    CAST(eq.fabricante AS VARCHAR(50)) AS fabricante,
    CAST(eq.generico AS VARCHAR(10)) AS generico,
    CAST(eq.marca AS VARCHAR(50)) AS marca,
    CAST(eq.blister AS VARCHAR(10)) AS blister,
    CAST(eq.elasticidad_demanda AS FLOAT) AS elasticidad_demanda,
