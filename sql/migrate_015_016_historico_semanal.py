#!/usr/bin/env python3
"""Apply sql/015 + 016: audit columns diario + Mercado_Historico_Semanal."""
...
def main() -> int:
    ...
    cur = conn.cursor()
    for name in ("015_mercado_historico_usd_audit.sql", "016_mercado_historico_semanal.sql"):
        text = (ROOT / name).read_text(encoding="utf-8")
        for batch in _split_batches(text):
            print(f"EXEC batch from {name} ({len(batch)} chars)…")
            cur.execute(batch)
    conn.close()
    print("OK 015+016 applied")
    return 0
